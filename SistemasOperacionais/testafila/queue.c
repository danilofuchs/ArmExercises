...
void queue_append(queue_t **queue, queue_t *element)
{
    ...
    {
        UARTprintf("### ERROR: Element already present in another queue\n");
        return;
    }
...
queue_t *queue_remove(queue_t **queue, queue_t *element)
{
    ...
    if (!queue)
    {
        UARTprintf("### ERROR: Cannot remove element from empty queue\n");
        debug_fn_return("NULL_QUEUE");
        return NULL;
    }
    if (!element)
    {
        UARTprintf("### ERROR: Element should be defined\n");
    ...
    {
        UARTprintf("### ERROR: Cannot remove element which is not in the queue\n");
